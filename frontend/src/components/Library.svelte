<script>
  ...
  async function refresh() {
    loading = true;
    try {
      sessionData = await getSession(params.sessionId);
    } finally {
      loading = false;
    }
  }
  ...
  async function handleDelete(materialId) {
    await deleteMaterial(params.sessionId, materialId);
    await refresh();
  }
...
<div class="navbar bg-base-100 shadow-sm">
  ...
  <div class="flex-none px-4">
    <a href="#/dashboard/{params.sessionId}" class="btn btn-ghost btn-sm">Back to Dashboard</a>
  </div>
</div>
...
<div class="container mx-auto p-4">
  ...
  {:else if sessionData?.materials?.length}
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {#each sessionData.materials as material}
